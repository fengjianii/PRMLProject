...
注意：
    当前共54个特征，其中20个用横截面rank替换了原始值
    rank特征消除股票间异质性，让模型学到"相对强弱"而非"绝对值"
    无量纲特征（收益率、波动率等）保留原始值
"""
...
class MeowFeatureGenerator(object):
    ...
    def featureNames(cls):
        ...
        返回：
            特征名称列表，共54个特征（20个用横截面rank替换原始值）
        ...
        return [
            # 基础特征（6个，3个用rank替换）
            "cs_rank_ob_imb0",        # ob_imb0横截面rank（替换原始值）
            "cs_rank_ob_imb4",        # ob_imb4横截面rank（替换原始值）
            "cs_rank_ob_imb9",        # ob_imb9横截面rank（替换原始值）
            "cs_rank_trade_imb",      # trade_imb横截面rank（替换原始值）
            "cs_rank_trade_imbema5",  # trade_imbema5横截面rank（替换原始值）
            "lagret12",               # 过去12分钟超额收益率（无量纲，保留原始值）
            
            # 价格动量/反转特征（12个，无量纲，保留原始值）
            "ret_1",          # 1分钟收益率
            "ret_3",          # 3分钟收益率
            "ret_6",          # 6分钟收益率
            "ret_12",         # 12分钟收益率
            ...
            # 盘口压力特征（16个，8个用rank替换，8个保留）
            "cs_rank_spread",                # spread横截面rank（替换）
            "cs_rank_relative_spread",       # relative_spread横截面rank（替换）
            "cs_rank_weighted_spread_4",     # weighted_spread_4横截面rank（替换）
            "cs_rank_amount_imb_4",          # amount_imb_4横截面rank（替换）
            "amount_imb_9",                  # 9档金额不平衡（保留，与4档不同）
            "amount_imb_19",                 # 19档金额不平衡（保留）
            "cs_rank_depth_sum_4",           # depth_sum_4横截面rank（替换）
            "depth_sum_9",                   # 9档总深度（保留）
            "cs_rank_depth_delta_4",         # depth_delta_4横截面rank（替换）
            "depth_delta_9",                 # 9档深度变化（保留）
            "cs_rank_buy_pressure_0",        # buy_pressure_0横截面rank（替换）
            "cs_rank_buy_pressure_4",        # buy_pressure_4横截面rank（替换）
            "buy_pressure_9",               # 买9档压力（保留）
            "cs_rank_micro_price_dev",       # micro_price_dev横截面rank（替换）
            "cs_rank_bid_ask_bias_0",        # bid_ask_bias_0横截面rank（替换）
            "bid_ask_bias_4",               # 买4卖4偏向（保留）
            
            # 成交主动性特征（15个，5个用rank替换，10个保留）
            "cs_rank_trade_buy_intensity",      # trade_buy_intensity横截面rank（替换）
            "trade_sell_intensity",             # 主卖强度（保留）
            "cs_rank_trade_net_intensity",      # trade_net_intensity横截面rank（替换）
            "cs_rank_trade_buy_turnover_ratio", # trade_buy_turnover_ratio横截面rank（替换）
            "trade_sell_turnover_ratio",        # 主卖成交额比例（保留）
            "trade_net_turnover_ratio",         # 净成交额比例（保留）
            "rolling_trade_buy_qty_6",          # 6窗口主买成交量（保留）
            "rolling_trade_buy_qty_12",         # 12窗口主买成交量（保留）
            "rolling_trade_sell_qty_6",         # 6窗口主卖成交量（保留）
            "rolling_trade_sell_qty_12",        # 12窗口主卖成交量（保留）
            "cs_rank_trade_count_imb",          # trade_count_imb横截面rank（替换）
            "trade_count_imbema5",              # 平滑成交笔数不平衡（保留）
            "cs_rank_buy_trade_size",           # buy_trade_size横截面rank（替换）
            "sell_trade_size",                  # 主卖单笔大小（保留）
            "trade_size_ratio",                 # 买卖单笔大小比（保留）
            
            # 交互特征（5个，使用rank版本）
            "ret_3_x_imb0",            # 短期动量 × 盘口不平衡rank
            "ret_6_x_imb0",            # 中期动量 × 盘口不平衡rank
            "ret_3_x_buy_intensity",   # 动量 × 买方力度rank
            "spread_x_vol",            # 流动性紧张rank × 高波动
            "highlow_x_imb0",          # 剧烈争夺 × 盘口偏向rank
        ]
