...
当前特征（共54个）：
    ...
    交互特征（5个）：
    50. ret_3_x_imb0: 短期动量 × 盘口不平衡
    51. ret_6_x_imb0: 中期动量 × 盘口不平衡
    52. ret_3_x_buy_intensity: 动量 × 买方力度
    53. spread_x_vol: 流动性紧张 × 高波动
    54. highlow_x_imb0: 剧烈争夺 × 盘口偏向

注意：
    当前共54个特征，涵盖了价格、盘口、成交三个维度的信息
"""
...
class MeowFeatureGenerator(object):
    ...
    def genFeatures(self, df):
        ...
        # 2. 中期动量 × 盘口不平衡
        df.loc[:, "ret_6_x_imb0"] = df["ret_6"] * df["ob_imb0"]
        
        # 3. 动量 × 买方力度
        df.loc[:, "ret_3_x_buy_intensity"] = df["ret_3"] * df["trade_buy_intensity"]
        
        # 4. 流动性紧张 × 高波动
        df.loc[:, "spread_x_vol"] = df["relative_spread"] * df["rolling_vol_12"]
        
        # 5. 剧烈争夺 × 盘口偏向
        df.loc[:, "highlow_x_imb0"] = df["high_low_range"] * df["ob_imb0"]
