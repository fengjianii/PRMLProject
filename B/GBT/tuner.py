...
class MeowTuner:
    ...
    def _suggest_params(self, trial):
        """
        定义超参数搜索空间（混合连续+离散，TPE高效探索）
        
        搜索空间设计原则：
        - 核心参数用suggest_int/float连续搜索，TPE能高效利用
        - 次要参数用categorical固定候选值，减少搜索维度
        - max_depth 3-6：深树极易过拟合
        - num_leaves随max_depth约束，不超过2^max_depth
        - learning_rate 0.005-0.05：太大过拟合
        - n_estimators 200-1000：让早停决定实际轮数
        - subsample/colsample 0.5-0.8：强采样抗过拟合
        - min_child_samples 50-500：大值抗过拟合
        - reg_alpha/lambda 0.01-5：轻到中等正则化
    ...
    def _train_and_evaluate_with_model(self, params, X_train, y_train, X_val, y_val,
        ...
        model.update_params({'early_stopping_rounds': early_stopping_rounds})
    ...
    def _objective(self, trial, X, y, n_splits=5, early_stopping_rounds=50,
        ...
        # 过拟合惩罚：OverfitRatio < 0.5 时，Pearson乘以OverfitRatio
        # 金融预测信号弱，0.5~0.9的OverfitRatio是正常水平，不惩罚
        # 只对严重过拟合(ratio<0.5)施加惩罚，使TPE避开极端参数
        if overfitting_ratio < 0.5 and avg_train_pearson > 0.02:
            log.yellow(f"Trial {trial.number}: overfitting ratio {overfitting_ratio:.3f} < 0.5, penalizing")
            avg_pearson *= overfitting_ratio
