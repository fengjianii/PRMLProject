...
模型使用 LightGBM GBT，经 Optuna 贝叶斯调参（50 轮，3 折 TimeSeriesSplit）。

### 4.1 性能对比

| 方案 | 特征数 | Pearson | 说明 |
|------|--------|---------|------|
| 原始 Ridge baseline | 6 | 0.029 | 项目原始代码 |
| V4（GBT + 54 特征） | 54 | 0.0635 | B 第一阶段最优 |
| **V5（GBT + 76 特征）** | **76** | **0.0714** | 当前最优，A+B 协作 |

V5 相比 baseline 提升 +147%，相比 V4 提升 +12.4%。

### 4.2 消融实验（A + B 协作确认）

| 特征集 | 特征数 | Pearson | 增量贡献 |
|--------|--------|---------|---------|
| V4 基线 | 54 | 0.0635 | — |
| V4 + P0（滚动统计） | 68 | — | +0.0079（估） |
| V4 + P1（横截面） | 62 | — | 正向 |
| V5 全量 | 76 | 0.0714 | 总计 +0.0079 |

P0 和 P1 各自独立都有正向贡献，且两类特征的信息源不重叠（P0 是时序动态，P1 是截面比较），加在一起效果最好。

### 4.3 关键调参发现（B 完成，A 参与讨论）

- **过拟合惩罚有害**：金融预测 OverfitRatio 0.4~0.6 属正常水平，惩罚会让 TPE 走向极端保守
- **5 折改 3 折**：5 折的第一折数据太少，Pearson 接近 0，污染均值
- **子采样从随机改为固定尾部截取**：不同 trial 评估不同时间窗口，CV 分数不可比
- **早停需与 lr 联动**：lr 越小需要越多的 patience 轮数
...
| `GBT-final/run_ablation.py` | **B** | 消融实验脚本 |
