...
消融实验确认：这个方案（V5）的 Pearson 为 0.0716，而改为全部 `_per_session` 的方案降到 0.0522。跨天连续性对金融时序特征至关重要。
...
项目原始代码使用 Ridge 回归。Ridge 的优势在于：(1) 训练快，秒级出结果，适合快速验证数据管线和评估逻辑；(2) L2 正则化对高维共线性特征有一定鲁棒性；(3) 作为线性模型，其结果可解释性强——如果 Ridge 就能做好的信号，说明线性关系已经足够。Ridge baseline 的 Pearson = 0.0222，说明原始 6 个特征在线性模型下预测力很弱，需要非线性模型和更丰富的特征。
...
| 实验3 | + P0 滚动统计 | GBT | 68 | 0.0683 | 0.0041 | 0.000024 | 核心特征的时序动态 |
| 实验4 | + P1 横截面 | GBT | 76 | **0.0716** | **0.0049** | 0.000024 | **V5 最终最优** |
...
- 训练集 Pearson: 0.132，验证集 Pearson: 0.074（早停点），测试集 Pearson: 0.0716
...
2. **泛化能力**：验证集Pearson=0.074，测试集Pearson=0.0716，泛化性能良好
